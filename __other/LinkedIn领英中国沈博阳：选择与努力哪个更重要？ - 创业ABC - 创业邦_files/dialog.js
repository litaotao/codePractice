...
(function ($, window, undefined) {
	...
	var artDialog = function (config, ok, cancel) {
		...
		if (elem && api)
			return api.follow(elem).focus();
		artDialog.PHPCMS_API = artDialog.PHPCMS_API || [];
		var that_api = artDialog.PHPCMS_API[config.id];
	...
	artDialog.fn = artDialog.prototype = {
		...
		content : function (msg, url) {
			...
			$content = DOM.content,
			content = $content[0],
			url = that.config.iframe || that.config.url,
			iframe = null,
			...
			if (isIframe) {
				if (typeof pc_hash == 'string')
					url.indexOf('pc_hash=') == -1 && (url += (url.indexOf('?') > -1 ? '&' : '?') + 'pc_hash=' + pc_hash);
				iframe = document.createElement('iframe');
				iframe.setAttribute("frameborder", 0, 0);
				iframe.setAttribute("width", '100%');
				iframe.setAttribute("height", '100%');
				iframe.setAttribute("id", iframrId);
				iframe.setAttribute("name", iframrId);
				iframe.src = url;
				msg = iframe;
				$ready = function () {
					that._reset(1);
					try {
						if (that.config.width !== 'auto' && that.config.height !== 'auto')
							return;
			...
			if (isIframe) {
				artDialog.PHPCMS_API[that.config.id] = msg.contentWindow;
				artDialog.data(that.config.id, msg.contentWindow)
			}
			that._ie6SelectFix();
		...
		lock : function () {
			...
			if (_isIE6)
				lockMask.html('<iframe src="about:blank" style="width:100%;height:100%;position:absolute;top:0;left:0;z-index:-1;filter:alpha(opacity=0)"></iframe>');
			lockMask.stop();
			lockMask.bind('click', function () {
				that._reset()
			}).bind('dblclick', function () {
				if (typeof(config.dblclickHide) == 'undefined' || config.dblclickHide == true) {
					that.close();
				}else{
					return false;
				}
...
(function ($, window, artDialog, undefined) {
	...
	$(function () {
		...
		style = mask.style;
		style.cssText = 'display:none;position:' + positionType + ';left:0;top:0;width:100%;height:100%;cursor:move;filter:alpha(opacity=0);opacity:0;background:#FFF';
