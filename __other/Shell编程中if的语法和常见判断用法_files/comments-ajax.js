	...
	function() {
		...
		$.ajax( {
			...
			success: function(data) {
				...
				new_htm = ( parent == '0' ) ? ('\n<ul style="clear:both;" class="comments comment-list' + new_htm + 'li>') : ('\n<ul class="children' + new_htm + 'ul>');
...
//////////////////////////////////////////////////////
	...
	$('body').on('click', '.comment-nav-below .nav-next a, .comment-nav-below .nav-previous a', function(e){
		...
		var postData = {
			'security':ajax_comment.nonce,
			'action': 'wortheme_get_comment_page',
			'get_page': getPage,
			'post_ID': $this.parents('#comments').prevAll('.post').data('id'),
		};
