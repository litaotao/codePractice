...
var a, dp = {
    sh: {
      ...
      ClipboardSwf: (function(){
        var protocol = location.href.match(/^https/i) ? 'https:' : 'http:';
        return protocol + '//csdnimg.cn/public/highlighter/ZeroClipboard.swf?e8a7c9a'
      }()),
      Version: "1.5.1"
    }
  };
...
$(document).ready(function () {
  var protocol = location.href.match(/^https/i) ? 'https:' : 'http:';
  $('<link rel="stylesheet" type="text/css" href="' + protocol + '//csdnimg.cn/public/highlighter/css/highlighter.css?e8a7c9a"/>').appendTo('head');

  $(".article_content pre, .news_content pre").each(function () {
    var $this = $(this);
    if ($this.attr("class").indexOf("brush:") != -1) {
      var lang = $this.attr("class").split(';')[0].split(':')[1];
      $this.attr('name', 'code');
      $this.attr('class', lang.replace(/^\s+|\s+$/g,''));
    }
    if ($this.attr("class")) {
      $this.attr('name', 'code');
    }
  });
  $('.article_content textarea[name=code], .news_content textarea[name=code]').each(function () {
    var $this = $(this);
  ...
  $("[code_snippet_id]").each(function () {
    var __s_id = $(this).attr("code_snippet_id");
