...
function _ts(n, d, p, t, o, c, u, a) {
    for (var id = 1; id <= d; id++) {
        ...
        if (u) {
            document.getElementById(u).href = a;
        }
    }
}
function DateChange(obj) {
    ...
    //alert(obj);
    var jsonString = '{"name":"James","birthday":"\\\/' + obj + '\\\/"}';
    jsonString = jsonString.replace(new RegExp('(^|[^\\\\])\\"\\\\/Date\\((-?[0-9]+)\\)\\\\/\\"', 'g'), "$1new Date($2)");
    //alert(jsonString);
    obj = eval('(' + jsonString + ')');
    return obj.birthday.toLocaleDateString();
    //alert(obj.birthday.toLocaleDateString());

}
function GetProjectList(list, id, len, time, count) {
    $.ajax({
        type: "get",
        url: "/Project/IniList?counts=" + count + "&id=" + id,
        cache: false,
        success: function (data) {
            $.each(data, (function (i) {
                if (time == true) {
                    var dt = DateChange(data[i].AddTime);
                    $("#" + list).append("<li><span>" + dt + "</span><a href='/Project/Detail/" + data[i].ID + "' target='_blank'>" + substr(data[i].Title, len) + "</a></li>");
                }
                else {
                    $("#" + list).append("<li><a href='/Project/Detail/" + data[i].ID + "' target='_blank'>" + substr(data[i].Title, len) + "</a></li>");
                }
            }));
...
function GetHotList(list, id,count) {
    $.ajax({
        type: "get",
        url: "/Project/IniHotList?counts="+count+"&id=" + id,
        cache: false,
        success: function (data) {
            $.each(data, (function (i) {
                $("#" + list).append("<li><a href='/Project/Detail/" + data[i].ID + "' target='_blank'>" + substr(data[i].Title, 20) + "</a></li>");
            }));
...
function IniInforHead(itemId) {
    $.ajax({
        type: "GET",
        url: "/Infor/InforCatagories",
        cache: true,
        success: function (data) {
            $.each(data, (function (i) {
                if (data[i].Code == itemId) {
                    $("#inforNavigation").append("<li ><a class='navactive' style='color:#ffffff' href='/Infor/List/" + data[i].Code + "' >" + data[i].Name + "</a></li>");
                }
                else {
                    $("#inforNavigation").append("<li><a href='/Infor/List/" + data[i].Code + "' >" + data[i].Name + "</a></li>");
                }
            }));
