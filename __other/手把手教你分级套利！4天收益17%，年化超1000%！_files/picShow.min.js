...
function picShow(options){var body=document.body,clientW=window.innerWidth||document.documentElement.clientWidth||document.body.clientWidth,clientH=document.documentElement.clientHeight||document.body.clientHeight||window.innerHeight,scrollTop=document.documentElement.scrollTop||document.body.scrollTop,android2_3up=navigator.userAgent.match(/Android ([3-9][._][0-9])/),ios5up=navigator.userAgent.match(/OS [5-9]_\d[_\d]*/),curSlider,tttt,iscrolling=false,opts={imgList:[],el:null,hidDom:options.domElement,preloadCount:2,curPage:1,showPage:true,showCtrlBtn:true,zanCfg:null,setGuide:true,maxW:null,maxH:null,zan:function(){},close:function(){}},localStorageSupported=function(){try{localStorage.setItem("test","test");localStorage.removeItem("test");return true;}catch(e){return false;}},isArray=function(obj){return Object.prototype.toString.call(obj)==='[object Array]';},addClass=function(element,MyclassName){element.className+=(element.className?" ":"")+MyclassName;},hasClass=function(node,MyClassName){return RegExp("(^|\\s)"+MyClassName+"(\\s|$)").test(node.className);},removeClass=function(node,className){node.className=node.className.replace(RegExp(className+"(\\s|$)"),"");},tranlate=function(x,y,z){curSlider.style.webkitTransform='translate('+x+'px,'+y+'px) translateZ(0)';},transition=function(prop,time,cup){curSlider.style.webkitTransition=prop+" "+time+"s "+cup;},setWrap=function(){var imgnum=opts.imgList.length||0,tpl='',ctrlBar;if(!opts.el){opts.el=document.createElement("div");opts.el.className="J-slider";body.appendChild(opts.el);}
tpl+='<div id="J-slider-cont">';for(var i=1;i<=imgnum;i++){tpl+='<div id="J-wrapper'+i+'" class="wrapper" style="-webkit-transform:translate3d(0,0,0);width:'+clientW+'px;height:'+clientH+'px"><div id="J-scroller'+i+'" class="scroller"></div></div>';}
tpl+='</div>';if(opts.showCtrlBtn){var postion=(ios5up||android2_3up&&navigator.userAgent.indexOf("UCBrowser")>0)?"fixed":"absolute";tpl+='<div class="ctrlbar" style="position:'+postion+'"><a id="J-zan" href="javascript:;"><i class="icon animated"></i><span id="J-zan-text" class="zan">赞</span><span id="J-zan-count" data-num="0"></span></a><a id="J-close" href="javascript:;">关闭</a>';if(opts.showPage&&opts.imgList.length>1){tpl+='<span id="J-curPage">'+opts.curPage+'</span><span id="J-pageCount">/'+imgnum+'</span>';}
tpl+='</div>';}
opts.el.innerHTML=tpl;opts.el.style.cssText='overflow:hidden;width:'+clientW+'px;height:'+clientH+'px';curSlider=document.getElementById("J-slider-cont");curSlider.style.cssText='position:relative;top:0px;left:0px;width:'+imgnum*clientW+'px;height:'+clientH+'px';if(navigator.userAgent.toLowerCase().indexOf("firefox")>0){document.querySelector(".ctrlbar").style.cssText="position:absolute;top:20px;left:0;";}},preloadImg=function(direction){var index,preImg,i;for(i=0;i<opts.preloadCount;i++){index=direction=="next"?opts.curPage+opts.preloadCount+i:opts.curPage-opts.preloadCount-i;preImg=document.getElementById('J-scroller'+index);if(preImg&&preImg.innerHTML==""){imgLoad(index);}}},loadScroll=function(wrapId){var wraper=document.getElementById(wrapId),disableMouse=false;disablePointer=false;if(navigator.userAgent.toLowerCase().indexOf("mobile")>0){disableMouse=true;disablePointer=true;}
var iscrol=new IScroll(wraper,{zoom:true,zoomMin:1,zoomMax:2,disableMouse:disableMouse,tap:"tap",scrollX:true,scrollY:true,mouseWheel:true,wheelAction:'zoom',doubleTapZoom:2,slider:opts.el});iscrol.on("flick",function(e){if(this.hasHorizontalScroll||Math.abs(this.distY)>Math.abs(this.distX)){return;}
if(this.distX>0){prev();}
else{next();}});return iscrol;},imgLoad=function(img_idx){var curScroll=document.getElementById("J-scroller"+img_idx),iscroll;if(curScroll){addClass(curScroll.parentNode,"loading");var img=new Image();img.onload=function(){if(curScroll){removeClass(curScroll.parentNode,"loading");curScroll.style.lineHeight=clientH+"px";iscroll.refresh();img.style.opacity="1";}}
img.src=opts.imgList[img_idx-1];document.getElementById("J-scroller"+img_idx).appendChild(img);iscroll=loadScroll('J-wrapper'+img_idx);}},setZan=function(init,zanCfg){zanCfg=zanCfg?zanCfg:opts.zanCfg;var zan=document.getElementById("J-zan");var zan_count=document.getElementById("J-zan-count");var zan_text=document.getElementById("J-zan-text");var icon=zan.getElementsByTagName("i")[0];var curZanCfg=isArray(zanCfg)?zanCfg[opts.curPage-1]:[zanCfg];var curPicWrap=document.getElementById("J-wrapper"+opts.curPage);if(init){if(zanCfg[0].disabled&&!hasClass(zan,"disabled")){addClass(zan,"disabled");}
else{removeClass(zan,"disabled");}
for(var i=1,len=zanCfg.length;i<=len;i++){var cur=document.getElementById("J-wrapper"+i);cur.setAttribute("pid",zanCfg[i-1].pid);cur.setAttribute("zanNum",zanCfg[i-1].like_counts);cur.setAttribute("zanLiked",zanCfg[i-1].liked);}}
removeClass(icon,"icon-like");removeClass(icon,"icon-liked");curPicWrap.getAttribute("zanLiked")=="true"?addClass(icon,"icon-liked"):addClass(icon,"icon-like");zan_count.innerHTML=curPicWrap.getAttribute("zanNum");if(parseInt(zan_count.innerHTML)>0){zan_text.style.display="none";zan_count.style.display="";}
else{zan_text.style.display="";zan_count.style.display="none";}},setGuide=function(){var guide=document.createElement("div");guide.className="J-guide";guide.style.cssText='height:'+clientH+'px;line-height:'+clientH+'px;opacity:0;-webkit-transition:opacity 0.6s ease';guide.innerHTML='<div class="J-guide-pop"></div>';opts.el.appendChild(guide);setTimeout(function(){guide.style.opacity="1";},1000);var time_guide=setTimeout(function(){opts.el.removeChild(guide);},5000);guide.onclick=function(){clearTimeout(time_guide);opts.el.removeChild(guide);}
if(opts.setGuide){try{if(localStorageSupported){window.localStorage['H5_IMGZOOM_GUIDE']="1";}}
catch(e){}}},hashChangeHandle=function(){var close=document.getElementById("J-close");var hashStr='#showPic',curHash=location.hash;if(!curHash||hashStr.indexOf(curHash)<0){opts.close.call(close);distroy();}},initEvent=function(){document.getElementById("J-close").onclick=function(){opts.close.call(this);distroy();}
document.getElementById("J-zan").onclick=function(){if(hasClass(this,"disabled")){return;}
opts.zan.call(this,opts.curPage);}},init=function(){if(!isArray(opts.imgList)){opts.imgList=[opts.imgList];}
if(opts.curPage){opts.curPage=parseInt(opts.curPage);}
else{opts.curPage=1;}
if(document.querySelector('.J-slider')){return;}
window.location.hash="showPic";window.onhashchange=hashChangeHandle;var windowMinH=Math.min(document.documentElement.clientHeight,window.innerHeight);clientH=windowMinH>0?windowMinH:clientH;setWrap();if(opts.zanCfg){setZan(1);}
else{document.getElementById("J-zan").style.display="none"}
if(opts.hidDom){for(var i=0;i<opts.hidDom.length;i++){opts.hidDom[i].style.display='none';}}
tranlate(-1*(opts.curPage-1)*clientW,0,0);setTimeout(function(){imgLoad(opts.curPage);imgLoad(opts.curPage-1);imgLoad(opts.curPage+1);initEvent();if(opts.setGuide){try{if(localStorageSupported()&&!window.localStorage['H5_IMGZOOM_GUIDE']){setGuide();}}
catch(e){}}},200);},gotoPage=function(){if(opts.zanCfg){setZan();}
if(opts.showPage){document.getElementById("J-curPage").innerHTML=""+opts.curPage;}
transition('all',0.3,'ease-out');tranlate(-1*(opts.curPage-1)*clientW,0,0);},prev=function(){if(opts.curPage>1){preloadImg("prev");opts.curPage--;gotoPage();}},next=function(){if(opts.curPage<opts.imgList.length){preloadImg("next");opts.curPage++;gotoPage();}},distroy=function(){if(opts.el){body.removeChild(opts.el);opts.el=null;if(opts.hidDom){for(var i=0;i<opts.hidDom.length;i++){opts.hidDom[i].style.display='block';}
if(scrollTop>0&&document.querySelectorAll("#detail-wrap").length==0){window.scrollTo(0,scrollTop);}}
if(location.hash=="#showPic"){history.go(-1);}}};for(var p in options){opts[p]=options[p];}
init();return{setZan:setZan,distroy:distroy,prev:prev,next:next}}
...
if(k==1){return 1;}
return(e*Math.pow(2,-10*k)*Math.sin((k-f/4)*(2*Math.PI)/f)+1);}}});me.tap=function(e,eventName){var ev=document.createEvent('Event');ev.initEvent(eventName,true,true);ev.pageX=e.pageX;ev.pageY=e.pageY;e.target.dispatchEvent(ev);};me.click=function(e){var target=e.target,ev;if(!(/(SELECT|INPUT|TEXTAREA)/i).test(target.tagName)){ev=document.createEvent('MouseEvents');ev.initMouseEvent('click',true,true,e.view,1,target.screenX,target.screenY,target.clientX,target.clientY,e.ctrlKey,e.altKey,e.shiftKey,e.metaKey,0,null);ev._constructed=true;target.dispatchEvent(ev);}};return me;})();function IScroll(el,options){this.wrapper=typeof el=='string'?document.querySelector(el):el;this.scroller=this.wrapper.children[0];this.scrollerStyle=this.scroller.style;this.options={slider:null,zoomMin:1,zoomMax:2,startZoom:1,resizeScrollbars:true,mouseWheelSpeed:20,snapThreshold:0.334,startX:0,startY:0,scrollY:true,directionLockThreshold:5,momentum:true,bounce:true,bounceTime:600,bounceEasing:'',preventDefault:true,preventDefaultException:{tagName:/^(INPUT|TEXTAREA|BUTTON|SELECT|IMG)$/},HWCompositing:true,useTransition:true,useTransform:true};for(var i in options){this.options[i]=options[i];}
...
var point=e.touches?e.touches[0]:e,deltaX=point.pageX-this.pointX,deltaY=point.pageY-this.pointY,timestamp=utils.getTime(),newX,newY,absDistX,absDistY;this.pointX=point.pageX;this.pointY=point.pageY;this.distX+=deltaX;this.distY+=deltaY;absDistX=Math.abs(this.distX);absDistY=Math.abs(this.distY);if(absDistX<10&&absDistY<10){return;}
...
this.scrollTo(newX,newY);if(!this.moved){if(this.doubleTapTimer&&this.options.zoom){clearTimeout(this.doubleTapTimer);this.doubleTapTimer=null;if(this.options.doubleTapZoom){this.zoom(this.scale>1?1:this.options.doubleTapZoom,point.pageX,point.pageY);}}else{var _that=this;this.doubleTapTimer=setTimeout(function(){_that.doubleTapTimer=null;if(_that.options.tap){utils.tap(e,_that.options.tap);}
if(_that.options.click){utils.click(e);}},this.options.zoom?250:0);}
...
this.endTime=0;this.directionX=0;this.directionY=0;this.wrapperOffset=utils.offset(this.wrapper);if(this.options.slider){this.wrapperOffset=utils.offset(this.options.slider);}
this._execEvent('refresh');this.resetPosition();},on:function(type,fn){if(!this._events[type]){this._events[type]=[];}
...
this.x=x;this.y=y;if(this.indicators){for(var i=this.indicators.length;i--;){this.indicators[i].updatePosition();}}},_initEvents:function(remove){var eventType=remove?utils.removeEvent:utils.addEvent,target=this.options.bindToWrapper?this.wrapper:window;eventType(window,'orientationchange',this);eventType(window,'resize',this);if(!this.options.disableMouse){eventType(this.wrapper,'mousedown',this);eventType(target,'mousemove',this);eventType(target,'mousecancel',this);eventType(target,'mouseup',this);}
...
this.isAnimating=true;step();},handleEvent:function(e){switch(e.type){case'touchstart':case'MSPointerDown':case'mousedown':this._start(e);if(this.options.zoom&&e.touches&&e.touches.length>1){this._zoomStart(e);}
